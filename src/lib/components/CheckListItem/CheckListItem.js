...
export class CheckListItem extends Component {
	...
	render() {
		return (
			<label htmlFor={this.props.title} className={this.state.selected === true ? ('check-list-item grid-flex gutter-tight no-break middle active') : ('check-list-item grid-flex gutter-tight no-break middle')}>
				{this.props.avatar ? (
